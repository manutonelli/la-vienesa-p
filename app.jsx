...
const toEmail  = u => `${u.trim().toLowerCase().replace(/\s+/g,'.')}@lavienesa.local`;
...
function Stepper({ value = 0, onChange, accent }) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:2 }}>
      ...
      <span style={{ minWidth:38, textAlign:'center', fontWeight:700, fontSize:16, color: value>0 ? (accent||C.accent) : C.muted }}>{value}</span>
...
function Modal({ open, onClose, title, children }) {
  if (!open) return null;
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.45)', display:'flex', alignItems:'flex-end', justifyContent:'center', zIndex:200 }} onClick={onClose}>
      <div style={{ background:C.surface, borderRadius:'20px 20px 0 0', width:'100%', maxWidth:640, maxHeight:'90dvh', overflow:'auto', boxShadow:'0 -8px 40px rgba(0,0,0,.12)' }} onClick={e => e.stopPropagation()}>
  ...
  return (
    <div style={{ position:'relative' }}>
      ...
      {open && filtered.length > 0 && (
        <div style={{ position:'absolute', top:'100%', left:0, right:0, zIndex:300, background:C.surface, border:`1.5px solid ${C.border}`, borderTop:'none', borderRadius:'0 0 10px 10px', maxHeight:220, overflow:'auto', boxShadow:'0 8px 24px rgba(0,0,0,.1)' }}>
          {filtered.map(o => (
...
function FAB({ onClick }) {
  return (
    <button onClick={onClick} style={{ position:'fixed', bottom:NAV_H+16, right:20, width:54, height:54, borderRadius:27, background:C.accent, color:'#fff', border:'none', fontSize:28, fontWeight:700, cursor:'pointer', zIndex:100, boxShadow:'0 4px 18px rgba(200,135,58,.45)', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'inherit' }}>+</button>
  );
}
function SectionHeader({ label }) {
  return <div style={{ padding:'10px 16px 6px', fontSize:11, fontWeight:800, color:C.muted, textTransform:'uppercase', letterSpacing:'.1em', background:C.bg, borderBottom:`1px solid ${C.border}` }}>{label}</div>;
}
...
function PrimaryBtn({ onClick, children, disabled, danger }) {
  return (
    <button onClick={onClick} disabled={disabled} style={{ width:'100%', padding:'13px', borderRadius:12, border:'none', background: disabled ? C.border : (danger ? C.danger : C.accent), color: disabled ? C.muted : '#fff', fontSize:15, fontWeight:700, cursor: disabled ? 'default' : 'pointer', fontFamily:'inherit', marginTop:8 }}>{children}</button>
  );
}
function Spinner() {
  return (
    <div style={{ minHeight:'100dvh', display:'flex', alignItems:'center', justifyContent:'center', background:C.bg }}>
      <div style={{ fontSize:14, color:C.muted, fontFamily:"'DM Sans',system-ui,sans-serif" }}>Cargando…</div>
    </div>
  );
}
function ErrorBanner({ msg }) {
  if (!msg) return null;
  return <div style={{ margin:'8px 0', padding:'10px 14px', borderRadius:10, background:C.dangerL, color:C.danger, fontSize:14 }}>{msg}</div>;
}
...
function LoginScreen() {
  ...
  return (
    <div style={{ minHeight:'100dvh', background:C.bg, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:24, fontFamily:"'DM Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background:C.surface, borderRadius:18, padding:'28px 24px', width:'100%', maxWidth:360, boxShadow:'0 4px 32px rgba(0,0,0,.07)' }}>
        ...
        <PrimaryBtn onClick={handleLogin} disabled={!usuario || !password || loading}>
...
/* ── Save Button ───────────────────────────────────────────────────────── */
function SaveButton({ tipo, data: formData }) {
  const { sucursal, fecha, perfil, session, setSavedStatus } = useCtx();
  ...
  useEffect(() => {
    ...
    setSavedInfo(null);
    sb.from('registros').select('guardado_por,guardado_en').eq('tipo',tipo).eq('sucursal',sucursal).eq('fecha',fecha).maybeSingle()
      .then(({ data }) => setSavedInfo(data || false));
  }, [tipo, sucursal, fecha]);
  ...
  async function handleSave() {
    ...
    const { error } = await sb.from('registros').upsert({
      ...
      guardado_por: perfil.nombre,
      guardado_en: new Date().toISOString(),
      user_id: session.user.id,
    }, { onConflict: 'tipo,sucursal,fecha' });
    if (!error) {
      const info = { guardado_por: perfil.nombre, guardado_en: new Date().toISOString() };
      setSavedInfo(info);
      setSavedStatus(prev => ({ ...prev, [tipo]: 'guardado' }));
      setFlash(true);
  ...
  return (
    <div style={{ position:'fixed', bottom:NAV_H, left:0, right:0, background:C.surface, borderTop:`1px solid ${C.border}`, padding:'10px 16px', display:'flex', alignItems:'center', gap:12, maxWidth:680, margin:'0 auto', zIndex:90, boxShadow:'0 -2px 12px rgba(0,0,0,.05)' }}>
      {savedInfo ? (
        <div style={{ flex:1, fontSize:12, color:C.muted }}>
          Guardado <strong style={{ color:C.ok }}>{fmtTime(savedInfo.guardado_en)}</strong> por {savedInfo.guardado_por}
        </div>
      ) : <div style={{ flex:1 }} />}
      <button onClick={handleSave} disabled={saving} style={{ padding:'11px 24px', borderRadius:12, border:'none', cursor: saving ? 'default' : 'pointer', background: flash ? C.ok : C.accent, color:'#fff', fontWeight:700, fontSize:14, fontFamily:'inherit', transition:'background .3s', flexShrink:0, display:'flex', alignItems:'center', gap:7 }}>
        {flash ? '✓ Guardado' : saving ? 'Guardando…' : 'Guardar planilla'}
      </button>
    </div>
  );
}
...
function HomeScreen({ setScreen }) {
  const { sucursal, setSucursal, fecha, setFecha, perfil, savedStatus, setShowUsers, setShowHistorial } = useCtx();
  const isEmpleado = perfil?.rol === 'empleado';

  const STATUS_LABEL = { guardado:'Guardado ✓', pendiente:'Pendiente' };
  const STATUS_COLOR = { guardado:C.ok,    pendiente:C.muted };
  const STATUS_BG    = { guardado:C.okL,   pendiente:C.faint };

  const cards = [
    { id:'comida',      label:'Planilla de Comida',       desc:'Entradas y salidas de productos' },
    { id:'devolucion',  label:'Devolución de Mercadería',  desc:'Registro de bajas' },
    { id:'sobreventas', label:'Sobreventas',               desc:'Alta y baja de sobreventas' },
    { id:'stock',       label:'Stock Lista',               desc:'Inventario de pastelería y cuartelería' },
  ];
    ...
    <div style={{ padding:'20px 16px', paddingBottom:NAV_H+20 }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:28 }}>
        <div>
          ...
          {isEmpleado ? (
            <div style={{ ...inp, color:C.muted, cursor:'default' }}>{sucursal}</div>
          ) : (
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
        {cards.map(c => {
          const status = savedStatus[c.id] || 'pendiente';
          return (
            <button key={c.id} onClick={() => setScreen(c.id)} style={{ display:'flex', alignItems:'center', gap:14, background:C.surface, border:`1.5px solid ${status === 'guardado' ? C.ok : C.border}`, borderRadius:14, padding:'15px 16px', cursor:'pointer', textAlign:'left', fontFamily:'inherit' }}>
              <div style={{ flex:1 }}>
              ...
              <Tag color={STATUS_COLOR[status]} bg={STATUS_BG[status]}>{STATUS_LABEL[status]}</Tag>
            </button>
          );
        })}

        <button onClick={() => setShowHistorial(true)} style={{ display:'flex', alignItems:'center', gap:14, background:C.faint, border:`1.5px dashed ${C.border}`, borderRadius:14, padding:'15px 16px', cursor:'pointer', textAlign:'left', fontFamily:'inherit', marginTop:4 }}>
        ...
        {perfil?.rol === 'admin' && (
          <button onClick={() => setShowUsers(true)} style={{ display:'flex', alignItems:'center', gap:14, background:C.faint, border:`1.5px dashed ${C.border}`, borderRadius:14, padding:'15px 16px', cursor:'pointer', textAlign:'left', fontFamily:'inherit' }}>
          ...
          {prods.map(p => {
            ...
            return (
              <div key={p.id} style={{ display:'grid', gridTemplateColumns:'1fr 130px 130px', alignItems:'center', padding:'10px 16px', borderBottom:`1px solid ${C.border}`, background:C.surface }}>
                ...
                <div style={{ display:'flex', justifyContent:'center' }}><Stepper value={v.entrada} onChange={val => updateProd(p.id, 'entrada', val)} accent={C.ok} /></div>
                <div style={{ display:'flex', justifyContent:'center' }}><Stepper value={v.salida} onChange={val => updateProd(p.id, 'salida', val)} accent={C.danger} /></div>
              </div>
            );
          })}
        </div>
    ...
    return items.map(r => (
      <div key={r.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'13px 16px', borderBottom:`1px solid ${C.border}`, background:C.surface }}>
        <div style={{ flex:1 }}>
          ...
          {r.fechaProducto && <div style={{ fontSize:12, color:C.muted, marginTop:2 }}>Fecha prod: {fmtDate(r.fechaProducto)}</div>}
        </div>
    ...
    <div style={{ paddingBottom:NAV_H+72 }}>
      ...
      {data.sobreventas.length === 0 ? (
        ...
        <>
          ...
          <List items={altas} color={C.ok} />
          <SectionHeader label={`Bajas (${bajas.length})`} />
          <List items={bajas} color={C.danger} />
        </>
      )}
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Nueva sobreventa">
        <Field label="Tipo">
          <div style={{ display:'flex', gap:8 }}>
            {['alta','baja'].map(t => (
              <button key={t} onClick={() => setForm(f => ({...f, tipo:t}))} style={{ flex:1, padding:'10px', borderRadius:10, cursor:'pointer', border:`1.5px solid ${form.tipo===t ? C.accent : C.border}`, background: form.tipo===t ? C.accentL : C.surface, color: form.tipo===t ? C.accentD : C.text, fontWeight:700, fontSize:14, fontFamily:'inherit', textTransform:'capitalize' }}>
                {t === 'alta' ? '↑ Alta' : '↓ Baja'}
              </button>
            ))}
          </div>
...
function StockScreen() {
  ...
  function ItemRow({ id, nombre }) {
    ...
    return (
      <div style={{ display:'grid', gridTemplateColumns:'1fr 90px 90px', alignItems:'center', padding:'9px 14px', borderBottom:`1px solid ${C.border}`, background:C.surface }}>
        ...
        <input type="number" value={v.cantidad} onChange={e => update(id,'cantidad',e.target.value)} placeholder="0" style={{ ...inp, padding:'7px 9px', fontSize:14, textAlign:'center' }} />
        <input type="number" value={v.peso} onChange={e => update(id,'peso',e.target.value)} placeholder="—" style={{ ...inp, padding:'7px 9px', fontSize:14, textAlign:'center', marginLeft:6 }} />
      </div>
    );
  }
  const tabBtn = (id, label) => (
    <button key={id} onClick={() => setTab(id)} style={{ flex:1, padding:'9px 4px', border:'none', cursor:'pointer', fontFamily:'inherit', background: tab===id ? C.accent : C.surface, color: tab===id ? '#fff' : C.muted, fontWeight: tab===id ? 700 : 500, fontSize:13, borderBottom:`2px solid ${tab===id ? C.accent : C.border}` }}>{label}</button>
  );
  const subBtn = (id, label) => (
    <button key={id} onClick={() => setSubTab(id)} style={{ padding:'6px 12px', borderRadius:20, border:'none', cursor:'pointer', fontFamily:'inherit', background: subTab===id ? C.accentL : 'transparent', color: subTab===id ? C.accentD : C.muted, fontWeight: subTab===id ? 700 : 500, fontSize:12, flexShrink:0 }}>{label}</button>
  );
  ...
  const colHeaders = (
    <div style={{ display:'grid', gridTemplateColumns:'1fr 90px 90px', padding:'7px 14px', background:C.faint, borderBottom:`1px solid ${C.border}` }}>
      {['Producto','Cantidad','Peso'].map((h,i) => <span key={i} style={{ fontSize:10, fontWeight:800, color:C.muted, textTransform:'uppercase', letterSpacing:'.06em', textAlign: i>0 ? 'center' : 'left' }}>{h}</span>)}
    </div>
  );
  return (
    <div style={{ paddingBottom:NAV_H+68 }}>
      ...
      {tab === 'cuarterio' && (
        <>
          {colHeaders}
          {STOCK_CUARTERIO.map((nombre,i) => <ItemRow key={i} id={`cu_${i}`} nombre={nombre} />)}
        </>
      )}
    </div>
  );
...
function BottomNav({ screen, setScreen }) {
  return (
    <div style={{ position:'fixed', bottom:0, left:0, right:0, height:NAV_H, background:C.surface, borderTop:`1px solid ${C.border}`, display:'flex', zIndex:150, boxShadow:'0 -2px 16px rgba(0,0,0,.06)' }}>
      {NAV_ITEMS.map(item => (
        <button key={item.id} onClick={() => setScreen(item.id)} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:2, border:'none', cursor:'pointer', background:'transparent', fontFamily:'inherit', color: screen===item.id ? C.accent : C.muted }}>
...
function RecordDetail({ tipo, data }) {
  if (tipo === 'comida') {
    ...
    if (!filled.length) return <div style={{ fontSize:13, color:C.muted }}>Sin movimientos</div>;
    return (
      <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
        {filled.map(p => (
          <div key={p.id} style={{ display:'flex', justifyContent:'space-between', fontSize:13 }}>
            <span style={{ color:C.text }}>{p.nombre}</span>
            <span><span style={{ color:C.ok }}>+{data[p.id].entrada}</span> / <span style={{ color:C.danger }}>-{data[p.id].salida}</span></span>
          </div>
        ))}
      </div>
    );
  ...
  if (tipo === 'devolucion') {
    ...
    return (
      <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
        {data.map((r,i) => <div key={i} style={{ fontSize:13, color:C.text }}><strong>{r.producto}</strong> — {r.peso}g · {r.motivo}{r.fechaElab ? ` · Elab: ${fmtDate(r.fechaElab)}` : ''}</div>)}
      </div>
    );
  }
  if (tipo === 'sobreventas') {
  ...
  if (tipo === 'stock') {
    ...
    return <div style={{ fontSize:13, color:C.muted }}>{keys.length} producto{keys.length!==1?'s':''} con datos</div>;
  }
  return null;
}
...
function HistorialScreen({ onBack }) {
  ...
  const [filtroSucursal, setFiltroSucursal] = useState(perfil?.rol === 'admin' ? 'Todas' : perfil?.sucursal);
  const [filtroTipo, setFiltroTipo] = useState('todos');
  const [expanded, setExpanded] = useState(null);

  useEffect(() => {
    let q = sb.from('registros').select('*').order('guardado_en', { ascending:false });
    if (perfil?.rol !== 'admin') q = q.eq('sucursal', perfil?.sucursal);
  ...
  const filtered = useMemo(() => records.filter(r => {
    ...
    return true;
  }), [records, filtroSucursal, filtroTipo]);

  const chipBtn = (active, label, onClick) => (
    <button onClick={onClick} style={{ padding:'5px 12px', borderRadius:20, border:'none', cursor:'pointer', background: active ? C.accent : C.faint, color: active ? '#fff' : C.muted, fontWeight: active ? 700 : 500, fontSize:12, fontFamily:'inherit', flexShrink:0 }}>{label}</button>
  );
      ...
      <div style={{ padding:'12px 14px', background:C.surface, borderBottom:`1px solid ${C.border}`, display:'flex', flexDirection:'column', gap:10 }}>
        {perfil?.rol === 'admin' && (
          <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
            {['Todas', ...SUCURSALES].map(s => chipBtn(filtroSucursal===s, s, () => setFiltroSucursal(s)))}
          </div>
        )}
        <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
          {[['todos','Todos'],['comida','Comida'],['devolucion','Devolución'],['sobreventas','Sobreventas'],['stock','Stock']].map(([id,label]) => (
            <button key={id} onClick={() => setFiltroTipo(id)} style={{ padding:'5px 12px', borderRadius:20, border:'none', cursor:'pointer', background: filtroTipo===id ? C.accentL : C.faint, color: filtroTipo===id ? C.accentD : C.muted, fontWeight: filtroTipo===id ? 700 : 500, fontSize:12, fontFamily:'inherit' }}>{label}</button>
          ))}
        </div>
      </div>

      {loading ? (
        <div style={{ padding:'60px 20px', textAlign:'center', color:C.muted }}>Cargando…</div>
      ) : (
        <div style={{ paddingBottom:32 }}>
          ...
          {filtered.map(r => (
            <div key={r.id}>
              <div onClick={() => setExpanded(expanded===r.id ? null : r.id)} style={{ display:'flex', alignItems:'center', gap:12, padding:'14px 16px', borderBottom:`1px solid ${C.border}`, background:C.surface, cursor:'pointer' }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontSize:12, color:C.muted, marginTop:2 }}>{fmtDate(r.fecha)} · {r.sucursal}</div>
                </div>
                <div style={{ textAlign:'right' }}>
                  <div style={{ fontSize:12, fontWeight:600, color:C.ok }}>{fmtDT(r.guardado_en).split(' ')[1]}</div>
                  <div style={{ fontSize:11, color:C.muted }}>{r.guardado_por}</div>
                </div>
              ...
              {expanded === r.id && (
                <div style={{ padding:'14px 16px', background:C.faint, borderBottom:`1px solid ${C.border}` }}>
                  <div style={{ fontSize:11, fontWeight:700, color:C.muted, textTransform:'uppercase', letterSpacing:'.06em', marginBottom:8 }}>Detalle</div>
...
function UsersScreen({ onBack }) {
  ...
  async function loadUsers() {
    ...
    const merged = (authUsers?.users || [])
      .filter(u => perfilMap[u.id])
      .map(u => ({ id:u.id, email:u.email, ...perfilMap[u.id] }));
    setUsers(merged);
    setLoading(false);
  }
  ...
  async function handleSave() {
    ...
    try {
      if (editId) {
        // Update perfil only
        await sb.from('perfiles').update({ nombre:form.nombre, rol:form.rol, sucursal: form.rol==='empleado' ? form.sucursal : null }).eq('id', editId);
        if (form.password) {
          await sbAdmin.auth.admin.updateUserById(editId, { password:form.password });
        }
      } else {
        if (!form.password) { setError('La contraseña es obligatoria'); setSaving(false); return; }
        const email = toEmail(form.usuario);
        const { data, error: authErr } = await sbAdmin.auth.admin.createUser({ email, password:form.password, email_confirm:true });
        if (authErr) { setError(authErr.message); setSaving(false); return; }
        await sb.from('perfiles').insert({ id:data.user.id, nombre:form.nombre, rol:form.rol, sucursal: form.rol==='empleado' ? form.sucursal : null });
      }
      await loadUsers();
      setOpen(false);
  ...
  async function handleDelete(userId) {
    ...
    await sb.from('perfiles').delete().eq('id', userId);
    await loadUsers();
    setDelConfirm(null);
  }

  function startEdit(u) {
    setForm({ nombre:u.nombre, usuario:u.email?.split('@')[0] || '', password:'', rol:u.rol, sucursal:u.sucursal || SUCURSALES[0] });
  ...
  return (
    <div style={{ minHeight:'100dvh', background:C.bg, fontFamily:"'DM Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ padding:'16px', paddingBottom:80 }}>
        {loading ? <div style={{ color:C.muted, fontSize:14 }}>Cargando…</div> : users.map(u => (
          <div key={u.id} style={{ background:C.surface, border:`1.5px solid ${C.border}`, borderRadius:14, padding:'14px 16px', marginBottom:10, display:'flex', alignItems:'center', gap:12 }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ fontSize:12, color:C.muted }}>@{u.email?.split('@')[0]}{u.sucursal && u.rol==='empleado' ? ` · ${u.sucursal}` : ''}</div>
            </div>
      ...
      <FAB onClick={() => { setForm({ nombre:'', usuario:'', password:'', rol:'empleado', sucursal:SUCURSALES[0] }); setEditId(null); setError(''); setOpen(true); }} />
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={editId ? 'Editar usuario' : 'Nuevo usuario'}>
        ...
        <div style={{ display:'flex', gap:'4%' }}>
          <Field label="Usuario" half>
            <input style={inp} value={form.usuario} onChange={e => setForm(f => ({...f, usuario:e.target.value.toLowerCase().replace(/\s/g,'')}))} placeholder="maria.garcia" autoCapitalize="none" disabled={!!editId} />
          </Field>
        ...
        <Field label="Rol">
          <div style={{ display:'flex', gap:8 }}>
            {['admin','empleado'].map(r => (
              <button key={r} onClick={() => setForm(f => ({...f, rol:r}))} style={{ flex:1, padding:'10px', borderRadius:10, cursor:'pointer', border:`1.5px solid ${form.rol===r ? C.accent : C.border}`, background: form.rol===r ? C.accentL : C.surface, color: form.rol===r ? C.accentD : C.text, fontWeight:700, fontSize:14, fontFamily:'inherit', textTransform:'capitalize' }}>{r}</button>
            ))}
          </div>
        </Field>
        {form.rol === 'empleado' && (
        ...
        <ErrorBanner msg={error} />
        <PrimaryBtn onClick={handleSave} disabled={!form.nombre || !form.usuario || saving}>{saving ? 'Guardando…' : 'Guardar'}</PrimaryBtn>
      </Modal>
      ...
      <Modal open={!!delConfirm} onClose={() => setDelConfirm(null)} title="Eliminar usuario">
        ...
        <div style={{ display:'flex', gap:10 }}>
          ...
          <button onClick={() => handleDelete(delConfirm)} style={{ flex:1, padding:12, borderRadius:12, border:'none', background:C.danger, color:'#fff', cursor:'pointer', fontFamily:'inherit', fontWeight:700 }}>Eliminar</button>
        </div>
      </Modal>
    </div>
  );
...
/* ── App root ──────────────────────────────────────────────────────────── */
const SCREEN_TITLES = {
  home:null, comida:'Planilla de Comida', devolucion:'Devolución de Mercadería',
  sobreventas:'Sobreventas', stock:'Stock Lista',
};
...
function App() {
  ...
  useEffect(() => {
    ...
    sb.from('perfiles').select('*').eq('id', session.user.id).single()
      .then(({ data }) => {
        if (!data) return;
        setPerfil(data);
        if (data.rol === 'empleado' && data.sucursal) setSucursal(data.sucursal);
      });
  }, [session]);

  /* Cargar datos guardados cuando cambia sucursal o fecha */
  useEffect(() => {
    if (!session || !sucursal || !fecha) return;
    setSavedStatus({});
    setData(defaultData());
    sb.from('registros').select('tipo,data,guardado_por,guardado_en').eq('sucursal', sucursal).eq('fecha', fecha)
      .then(({ data: records }) => {
        if (!records || !records.length) return;
        const newData = defaultData();
        const newStatus = {};
        records.forEach(r => { newData[r.tipo] = r.data; newStatus[r.tipo] = 'guardado'; });
        setData(newData);
        setSavedStatus(newStatus);
      });
  }, [session, sucursal, fecha]);

  function setDataKey(key, val) { setData(d => ({ ...d, [key]: val })); }
  ...
  const ctx = { session, perfil, sucursal, setSucursal, fecha, setFecha, data, setDataKey, savedStatus, setSavedStatus, setShowUsers, setShowHistorial };

  if (showUsers && perfil.rol === 'admin') return <Ctx.Provider value={ctx}><UsersScreen onBack={() => setShowUsers(false)} /></Ctx.Provider>;
  ...
  return (
    <Ctx.Provider value={ctx}>
      <div style={{ minHeight:'100dvh', background:C.bg, fontFamily:"'DM Sans',system-ui,sans-serif", color:C.text, maxWidth:680, margin:'0 auto', position:'relative' }}>
        {/* Top bar */}
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 18px 10px', background:C.surface, borderBottom:`1px solid ${C.border}` }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            ...
            <button onClick={handleLogout} title="Cerrar sesión" style={{ background:C.faint, border:'none', borderRadius:20, padding:'5px 11px', fontSize:11, fontWeight:700, color:C.muted, cursor:'pointer', fontFamily:'inherit', display:'flex', alignItems:'center', gap:5 }}>
